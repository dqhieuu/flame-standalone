...
class FLAME(nn.Module):
    ...
    def __init__(self, config, optimize_basis=False):
        ...
        shapedirs = torch.cat(
            [shapedirs[:, :, :self.cfg.n_shape], shapedirs[:, :, 300:]], 2)
        ...
        self.register_parameter('eye_pose', nn.Parameter(
            default_eyball_pose, requires_grad=False))
        default_neck_pose = torch.zeros(
            [1, 3], dtype=self.dtype, requires_grad=False)
        self.register_parameter('neck_pose', nn.Parameter(
            default_neck_pose, requires_grad=False))

        # Static and Dynamic Landmark embeddings for FLAME
        lmk_embeddings = np.load(
            self.cfg.flame_lmk_embedding_path, allow_pickle=True, encoding='latin1')
        lmk_embeddings = lmk_embeddings[()]
        self.register_buffer('lmk_faces_idx', torch.from_numpy(
            lmk_embeddings['static_lmk_faces_idx']).long())
        self.register_buffer('lmk_bary_coords', torch.from_numpy(
            lmk_embeddings['static_lmk_bary_coords']).to(self.dtype))
        self.register_buffer('dynamic_lmk_faces_idx',
                             lmk_embeddings['dynamic_lmk_faces_idx'].long())
        self.register_buffer('dynamic_lmk_bary_coords',
                             lmk_embeddings['dynamic_lmk_bary_coords'].to(self.dtype))
        self.register_buffer('full_lmk_faces_idx', torch.from_numpy(
            lmk_embeddings['full_lmk_faces_idx']).long())
        self.register_buffer('full_lmk_bary_coords', torch.from_numpy(
            lmk_embeddings['full_lmk_bary_coords']).to(self.dtype))
    ...
    def compute_distance_to_basis(self, shape_vector, shape_as_offset=False):
        ...
        if shape_as_offset:
            ...
            diff = shape_vector - self.v_template
        shape_params = torch.matmul(diff.reshape(
            batch_size, -1), self.shapedirs[:, :, :n_eigenvectors].reshape(3 * n_vertices, n_eigenvectors))
        distance = diff - torch.matmul(shape_params, self.shapedirs[:, :, :n_eigenvectors].reshape(
            n_vertices * 3, n_eigenvectors).t()).reshape(batch_size, n_vertices, 3)
        return distance
...
class Cfg:
    def __init__(self):
        self.flame_model_path = './generic_model.pkl'
...
if __name__ == '__main__':
    ...
    # with open(cfg.flame_model_path, "rb") as f:
